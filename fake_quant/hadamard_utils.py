...
def get_hadK(n, transpose=False):
    ...
    elif n % 148 == 0:  # Qwen2.5 7B intermediate
    ...
    if had_dim == -1:
        if output:
            ...
            if bias_ is not None:
                bias_ = matmul_hadU_cuda(bias_.unsqueeze(0), had_K, K).reshape(-1)
        if not output:
        ...
        if output:
            ...
            if bias_ is not None:
                bias_ = bias_.unsqueeze(0).cuda()
                bias_ = fast_hadamard_transform.hadamard_transform(
                    bias_.reshape(-1, bias_.shape[-1]//had_dim, had_dim),
                    scale=1/math.sqrt(had_dim)
                    ).reshape(-1)
        else:
